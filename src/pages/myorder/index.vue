  ...
  <div class="wrap">
    ...
    <div class="main" v-else>
      <div class="shop" @click="goOrderdetails(item)" v-for="(item, index) in storeList" :key="index">
        ...
        <dl class="shop-box" v-for="(value, key) in item.products" :key="key">
          <dd>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      storeList(){
        this.storelist.forEach(item=>{
          ...
          item.orderType2 = item.processStatus
          item.processStatus = item.cancleStatus==1?'已取消':item.processStatus==1?'待付款':item.processStatus==2?'待发货':item.processStatus==3?'待收货':item.processStatus==4?'待评价':item.processStatus==5?'取消':item.processStatus==6?'已退款':item.processStatus==9?'交易完成':'完成';
        })
    ...
    methods: {
      ...
      goPayment(item){
         wx.navigateTo({
          url: '/pages/myorder/settlement/main?orderId='+item.orderId+'&totalAmount='+item.totalAmount+'&createTime='+item.createTime
        });
      },
      // 确认收货
      ensure(item){
        wx.showModal({
          ...
          success: async (result) => {
            if (result.confirm) {
              let result = await ensureOrder({subOrderId: item.orderId})
    ...
    onLoad(options) {
      this.orderStatus = options.orderStatus || 0;
      //判断网路状态
      wx.getNetworkType({
        success: res => {
      ...
      this.fetchData(this.orderStatus);
    },
    onShow(){
