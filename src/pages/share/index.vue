<template>
  <div class="ShareWrap">
    <h2>{{info.title}}</h2>
    <div class="price">【在售价】{{info.salesPrice}}元</div>
...
import {getDetail,shareNum} from '@/api/index';
...
export default {
  ...
  async onLoad(options){
    ...
    this.onImgIndex = 0
    let that = this
    let result = await getDetail({pid: this.id});
    ...
    this.imgMap = [{url:result.data.result.mainImgUrl,on:true}]
    if(this.info.supplierProductPictureVoList&&this.info.supplierProductPictureVoList.length){
      this.info.supplierProductPictureVoList.map(val=>{
  ...
  onShareAppMessage(res) {
    ...
    // let shareRes = await shareNum()
    console.log('计数+1')
    if (res.from === 'button') {
      // 来自页面内转发按钮
      console.log(res.target)
    }
    console.log('this.inviteCodeData',inviteCode)
    return {
      title: that.info.title,
      imageUrl:that.info.mainImgUrl,
      path: 'pages/go2detail/main?id='+that.info.pid+'&fromInviteCode='+inviteCode
    }
  },
  methods: {
    // 画图
   ...
   console.log('画布宽度',canW,canH)
    ...
    context.draw(true, async ()=>{
      ...
      // 绘制商品
      console.log('改变为base64',this.info.mainImgUrl+'')
      wx.showLoading({
        title: '下载详情图中', //提示的内容,
        mask: true, //显示透明蒙层，防止触摸穿透,
      });
      ...
      if(imgToBase64Url.data.res_code==1){
        ...
        // imgPath = await downFile(this.info.mainImgUrl);
      }
      ...
      if(this.info.productType==2){
        ...
        context.fillText(this.info.seckillPrice,40,canH-25,160,90)
        context.restore();
      }else{
        ...
        context.setFillStyle('red')
        context.setFontSize(10)
        context.fillText(this.info.salesPrice,20,canH-25,160,90)
      }
      context.restore();
      ...
      console.log('二维码路径',this.info.xcxFriendShareUrl)
      qrCodeJs.api.draw(that.info.xcxFriendShareUrl, 'canvas', 300, 300);
      console.log('生成')
      setTimeout(() => {
        wx.showLoading({
          title: '绘制二维码中', //提示的内容,
    ...
    saveImageFun(index){
      let that = this
      console.log('saveImageFun',index)
      wx.canvasToTempFilePath({
          x: 0,
          y: 0,
          // width: 197,
