<template>
  <div class="wrap">
    ...
    <div class="refer">
      <input v-model="value" focus maxlength="8" placeholder="请输入邀请码">
      <button :class="status?'able':''" @click="jionGroup(group)">提交</button>
    </div>
    <div @click="jionGroup(group)" class="visitor" :id="showGroup?'':'show'">
        <dl>
          <dd>
    ...
    <!-- <div v-if="listInvite&&listInvite.length" class="hint">- 无邀请码可选择以下团队加入 -</div> -->
    <ul class="list">
      <li v-for="(item,index) in listInvite" :key="index">
        <image :src="item.headUrl" mode="widthFix" alt=""></image>
        <div class="content">
          <p class="name">{{item.nickName}}</p>
          <!-- <p class="group">团队人数: {{item.totalInviteNumber}}</p> -->
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async jionGroup(data){
        ...
        if(dataResult.data.res_code==1){
          ...
          console.log('loginResult222====',loginResult)
          wx.setStorage({key:'loginResult',data:loginResult})
          let isLoginUrl = wx.getStorageSync('loginUrl')
          if(!isLoginUrl){
    ...
    async onLoad() {
      let fromInviteCode = wx.getStorageSync('fromInviteCode')
      console.log('获取code============',fromInviteCode)
      if(fromInviteCode){
        this.value = fromInviteCode
        this.submit()
      }
